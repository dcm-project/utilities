        ...
        for i in $(seq 0 $((PROV_COUNT - 1))); do
            ...
            case "${PROV_FLAGS[$i]}" in
                k8s-storage-service-provider)
                    ...
                    ;;
                environment-agent)
                    printf 'export DCM_ENVIRONMENT_AGENT_URL=%q\n' "http://localhost:8090/api/v1alpha1"
                    printf 'export K8S_STORAGE_SP_REGISTERED_ENDPOINT=%q\n' "embedded://storage"
                    printf 'export DCM_NATS_URL=%q\n' "nats://localhost:4222"
                    wrote=true
                    ;;
            esac
...
validate_environment_agent_provider() {
    log "Validating environment-agent prerequisites"
    ensure_provider_namespace "$1" "$2" "$3"

    local embedded_sps="${AGENT_EMBEDDED_SPS:-storage}"
    info "Configured embedded SPs (AGENT_EMBEDDED_SPS): ${embedded_sps}"
    if [[ ",${embedded_sps}," == *",storage,"* ]]; then
        ensure_storage_class_for_e2e "$1" "$3"
    fi
}
            ...
            if match_provider_flag "$1"; then
                case "${MATCHED_TYPE}" in
                    ...
                    namespace)
                        require_arg "$1" "${2:-}"
                        ns="${2:-}"
                        matched_flag="${PROV_NS_FLAGS[MATCHED_IDX]}"
                        for i in $(seq 0 $((PROV_COUNT - 1))); do
                            if [[ "${PROV_NS_FLAGS[$i]}" == "${matched_flag}" ]]; then
                                PROV_NAMESPACES[i]="${ns}"
                            fi
                        done
                        shift 2 ;;
                esac
