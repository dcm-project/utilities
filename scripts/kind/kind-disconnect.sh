#!/usr/bin/env bash
# Disconnect Kind from compose networks before compose-down.
set -euo pipefail
...
if [[ -n "${COMPOSE_NETWORK:-}" ]]; then
	...
	NETWORKS="${COMPOSE_NETWORKS:-environment-agent_default control-plane_default deploy_default dcm-e2e_default}"
fi
